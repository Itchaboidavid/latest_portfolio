  ...
  return (
    <section className='flex max-w-4xl flex-col gap-12 pt-20' id='bio'>
        ...
        <motion.div initial={{opacity: 0}} whileInView={{opacity: 1}} transition={{duration: 1}}>
            {BIO.map((bio, index) => (
                <motion.p initial={{opacity: 0, x: -20}} whileInView={{opacity: 1, x: 0}} transition={{duration: 0.8, delay: index * 0.5}} key={index} className='mb-4 text-lg lg:text-xl text-justify px-2'>{bio}</motion.p>
            ))}
        </motion.div>
    </section>
  )
